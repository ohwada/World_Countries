...
FILE_PREF_GEOJSON = 'data/yamanashi.geojson'

FILE_PLACE_LIST = 'data/yamanashi_place_list.json'
...
with open(FILE_PLACE_LIST, 'r') as f1:
    dic1 = json.load(f1)
#

list_places = dic1['places']
...
def within(geo):
    ...
    name_place = ''
    name_municipality = ''
    for item1 in list_places:
        name_en = item1['name_en']
        name_place = item1['name_place']
        ...
        if point.within(geo):
            is_within = True
            break
    return [is_within, name_en, name_place, name_municipality]
#
