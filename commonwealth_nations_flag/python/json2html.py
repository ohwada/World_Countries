...
with open('commonwealth_nations_flag.json') as f3:
    dic = json.load(f3)
    str_title = dic['title']
    desc = dic['desc']
    ref = dic['reference']
    ...
    for item in list_countries:
        ...
        row = template_row.format(country=row_country, flag_icon=row_flag_icon, date=date, region=row_region, subregion=row_subregion, population=population, government=government,notes=notes)
        print(row)
        rows +=  row
#
...
with open('commonwealth_nations_flag.html', 'w') as f4:
    f4.write(wdata)
