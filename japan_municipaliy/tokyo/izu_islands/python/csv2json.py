# -*- coding: utf-8 -*-
# csv2json.py
# 2023-06-01 K.OHWADA

import csv
import json


FILE_CSV = 'izu_islands.csv'

FILE_JSON = 'izu_islands_list.json'


COMMA = ','

COMMA_HTML = '&comma;'


def  restore_comma(data):
    if not data:
        return ""
    str_data = data.strip()
    ret = str_data.replace(COMMA_HTML, COMMA)
    return ret
#


dic = {}

dic['title'] = 'List of Izu islands'

dic['title_ja'] = '伊豆諸島の島の一覧'

dic['reference'] = 'wikipedia: 伊豆諸島'

dic['url_reference'] = 'https://ja.wikipedia.org/wiki/%E4%BC%8A%E8%B1%86%E8%AB%B8%E5%B3%B6'


islands = []


with open(FILE_CSV, 'r') as f1:
    reader = csv.reader(f1)

    for row in reader:
        len_row = len(row)
        print('len: ', len_row)
        if len_row < 11:
            continue
        d= {}
        d['name_island'] = row[0].strip()
        d['name_en'] = row[1].strip()
        d['url_island'] = row[2].strip()
        d['name_town'] = row[3].strip()        
        d['url_town'] = row[4].strip()
        d['population'] = restore_comma( row[5] )
        d['area'] = row[6].strip()
        d['lat'] = float( row[7].strip() )
        d['lon'] = float( row[8].strip() )
        d['name_airport'] = row[9].strip()        
        d['url_airport'] = row[10].strip()
        print(d)
        islands.append(d)
#


dic['islands'] = islands


with open(FILE_JSON, 'wt',  encoding='utf-8') as f2:
    json.dump(dic, f2, ensure_ascii=False)
#

