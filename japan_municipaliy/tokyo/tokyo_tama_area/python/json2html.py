...
FILE_HTML = 'tokyo_tama_area_list.html'
...
for item in list_cities :
    ...
    lat = item['lat']
    lon = item['lon']
    row_city = FORMAT_A_TAG.format(href=url_city, name=name_ja)
    row_flag = FORMAT_IMG.format(src=url_flag, width=flag_width, height=flag_height)
    row_coordinates = make_coordinates(lat, lon)
    row = template_row.format(number = number,  code=code, city=row_city, flag=row_flag, population=population, area =area, density=density, revenue=revenue, established=established, name_en=name_en, coordinates= row_coordinates, city_width= CITY_WIDTH )
    print(row)
    rows +=  row
#
